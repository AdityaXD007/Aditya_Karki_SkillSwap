...
import { matchesApi, bookingsApi, type Match, type Booking } from '@/services/api';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setUpcomingBookings(bookingsData.filter(b => b.status === 'confirmed').slice(0, 3));
      } catch (error) {
                  ...
                  upcomingBookings.map((booking) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-gray-900 truncate">{booking.skill}</p>
                        <p className="text-sm text-gray-500">with {booking.partnerName}</p>
                      </div>
              ...
              <div className="space-y-4">
                {matches.map((match) => (
                  <div key={match.id} className="flex items-center space-x-3">
                    ...
                    />
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-gray-900 text-sm truncate">{match.name}</p>
